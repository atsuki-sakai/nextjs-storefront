            ...
            <div className='tw-grid tw-grid-cols-3 tw-gap-4 tw-place-items-center tw-m-8'>
                {
                    products.map(product => (
                        <div className=' tw-bg-gray-100 tw-p-8 tw-shadow-xl tw-rounded-l -'>
                            <div className='tw-flex tw-justify-center'>
                                <Link href={`/products/${product.handle}`}>
                                    <a>
                                        <Image className='tw-w-auto tw-h-56' src={product.variants[0].image.src} alt={product.title} layout='fill' objectFit='cover' />
                                    </a>
                                </Link>
                            </div>
                            <div className="tw-m-4">
                                <h1 className='tw-font-bold tw-text-l tw-text-clip tw-w-2rem'>{product.title}</h1>
